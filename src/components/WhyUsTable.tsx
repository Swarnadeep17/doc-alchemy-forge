...
export const WhyUsTable = () => (
  <section className="w-full max-w-2xl mx-auto mt-12 mb-10 animate-fade-in">
    <h2 className="text-xl font-bold text-white/90 font-mono mb-3 tracking-wide">Why Docenclave vs. Others?</h2>
          ...
          {differentiators.map(([feature, ours, theirs], idx) => (
            <tr key={feature} className="border-b border-white/5 hover:bg-white/5 transition-colors">
              ...
              <td className="py-2 px-2 text-center">
                <span className="inline-block rounded font-bold bg-black/60 border border-white/10 px-3 py-1">{ours}</span>
              </td>
              <td className="py-2 px-2 text-center">
                <span className="inline-block rounded font-bold bg-white/5 text-white/50 border border-white/10 px-3 py-1">{theirs}</span>
              </td>
            </tr>
          ))}
        </tbody>
