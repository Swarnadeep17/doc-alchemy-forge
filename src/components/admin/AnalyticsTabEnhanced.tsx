...
import { AreaChart, Area, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid, Legend, BarChart, Bar, PieChart, Pie, Cell } from "recharts";
...
const AnalyticsTabEnhanced = () => {
  ...
  const toolStats = useMemo(() => {
    const result: Array<{ name: string; category: string; visits: number; downloads: number }> = [];
    ...
    return result.sort((a, b) => b[selectedMetric] - a[selectedMetric]);
  }, [stats.tools, selectedMetric]);
          ...
          <CardContent className="p-6">
            <div className="flex justify-between items-start">
              <div>
                ...
                <h3 className="text-2xl font-bold text-white mt-2">{categoryStats.length}</h3>
              </div>
        ...
        <div className="flex gap-2">
          {TIME_RANGES.map((range) => (
            <Button
              ...
              className={timeRange === range.value ? "bg-cyan-600 hover:bg-cyan-700" : ""}
            >
              {range.label}
            </Button>
          ))}
...
export default AnalyticsTabEnhanced;
